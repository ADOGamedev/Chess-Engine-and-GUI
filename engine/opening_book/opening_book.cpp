...
bool is_position_in_book(const uint64_t& key) {
    return binary_search(key, 0, book_positions.size() - 1) != -1;
}


std::string get_random_move(const uint64_t& key) {
    int index = binary_search(key, 0, book_positions.size() - 1);
    BookPosition& book_pos = book_positions[index];

    int total_weight = std::accumulate(book_pos.weights.begin(), book_pos.weights.end(), 0);
    int sum = 0;
    int random = randi_range(1, total_weight);

    for (int i = book_pos.weights.size() - 1; i >= 0; i--) {
        sum += book_pos.weights[i];
        if (random <= sum) {
            return book_pos.lan_moves[i];
        }
    }
}
...
int binary_search(const uint64_t& key, const int begin, const int end) {
    ...
    if (key < book_positions[middle].key) {
        return binary_search(key, begin, middle - 1);
    }
    else if (key > book_positions[middle].key) {
